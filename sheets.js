...
async function getSheetData(auth, rowNumber = 2) {
  ...
  const range = `'CREATE PDF Form'!A${rowNumber}:AW${rowNumber}`;
  ...
  const dateRecipientRaw = getValue(row, 37);
  const dateSignerRaw = getValue(row, 43);
  const dateStaffRaw = getValue(row, 45);

  const EXECUTIVE_Name = getValue(row, 35);
  const RECIPIENT_Name = getMatchedValue(getValue(row, 38));
  const SIGNER_Name = getMatchedValue(getValue(row, 44));
  const PPSTAFF_Name = getValue(row, 47);
  ...
  return {
    ...
    FORM_EXECUTIVE: EXECUTIVE_Name,
    FORM_EXECUTIVE_DF: removeTitle(EXECUTIVE_Name),
    FORM_EXECUTIVE_VACANCY: getValue(row, 36),

    RECIPIENT_TEMP: formatThaiDate(dateRecipientRaw).replace(/ /g, " / "),
    FORM_RECIPIENT: RECIPIENT_Name,
    FORM_RECIPIENT_VACANCY: getAfterParenthesis(getValue(row, 38)),
    FORM_CB_ACCEPT: getValue(row, 39) === "TRUE",
    FORM_CB_DENY: getValue(row, 39) === "FALSE",
    FORM_DENY_CAUSE: getValue(row, 40),
    FORM_TO_SIGNER: getValue(row, 36) ? "อ" + getValue(row, 36).substring(1) : "",
    FORM_APPROVED: getValue(row, 41),
    FORM_UNITS: getValue(row, 42),

    SIGNER_TEMP: formatThaiDate(dateSignerRaw).replace(/ /g, " / "),
    FORM_SIGNER: SIGNER_Name,
    FORM_SIGNER_VACANCY: getValue(row, 44)?.replace(/\([^)]+\)/, "").trim() || "",

    PPSTAFF_TEMP: formatThaiDate(dateStaffRaw).replace(/ /g, " / "),
    PERFORMANCE_REPORT: getValue(row, 46),
    FORM_PPSTAFF: PPSTAFF_Name,
    FORM_PPSTAFF_DF: removeTitle(PPSTAFF_Name),
    FORM_PPSTAFF_VACANCY: getValue(row, 48),

    SIGNATURE_EXECUTIVE: getSignatureUrlByName(EXECUTIVE_Name),
    SIGNATURE_RECIPIENT: getSignatureUrlByName(RECIPIENT_Name),
    SIGNATURE_SIGNER: getSignatureUrlByName(SIGNER_Name),
  };
}
