...
async function getSheetData(auth, rowNumber = 2) {
  ...
  // เช็ค log base64 สำหรับ signature เฉพาะของผู้รับ
  const name = getMatchedValue(getValue(row, 37));
  logger.info(`ชื่อผู้รับ: ${name}`);
  logger.info(`BASE64: ${getSignatureBase64(name).substring(0, 50)}...`);
  logger.info(`Raw value: ${getValue(row, 37)}`);
  logger.info(`Cleaned name: ${name}`);
  ...
  return {
    ...
    SIGNATURE_EXECUTIVE: getSignatureBase64(getValue(row, 35)),
    SIGNATURE_RECIPIENT: "https://drive.google.com/uc?export=view&id=1sVhQ2IEpK78R7NmmR0DccNJIzT8zzdUs",
    SIGNATURE_RECIPIENT: getSignatureBase64(getMatchedValue(getValue(row, 37))),
    SIGNATURE_SIGNER: getSignatureBase64(getMatchedValue(getValue(row, 42))),
    SIGNATURE_PPSTAFF: getSignatureBase64(getValue(row, 44)),
  };
}
