...
async function getSheetData(auth, rowNumber = 2) {
  ...
  const range = `'CREATE PDF Form'!A${rowNumber}:AC${rowNumber}`;
  ...
  return {
    ...
    FORM_CB_SUBSTATION: values[13] === "Substation",
    FORM_CB_UNMANSUBSTATION: values[13] === "Unmanned Substation",
    FORM_CB_AGENCY: true ,
    FORM_CB_EXTERNALAGENCY: values[7],
    FORM_CB_EXTINGUISH: values[12] == "ดับไฟปฏิบัติงาน",
    FORM_CB_NOEXTINGUISH: values[12] == "ไม่ดับไฟปฏิบัติงาน",
    FORM_CB_PLAN: values[14] == "นอกแผน",
    FORM_CB_NOPLAN: values[14] == "ตามแผน",
    FORM_CB_EMERGENCY: values[14] == "กรณีฉุกเฉิน (ศูนย์ฯ พิจารณา)",
    // WP_NUM: ,
    // WP_YEAR: ,
    FORM_EXECUTIVE: values[30],
    FORM_EXECUTIVE_VACANCY: values[31],
