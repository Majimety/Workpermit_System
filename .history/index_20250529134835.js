...
async function deleteOldFileIfExists(authClient, rowNumber) { //ลบไฟล์เก่า
  const sheets = google.sheets({ version: "v4", auth: authClient });
  const range = `CREATE PDF Form!AE${rowNumber}`;
...
async function updateSheetWithUrl(authClient, rowNumber, url) { //เพิ่มไฟล์ใหม่
  const sheets = google.sheets({ version: "v4", auth: authClient });
  await sheets.spreadsheets.values.update({
    spreadsheetId: process.env.SHEET_ID,
    range: `CREATE PDF Form!AE${rowNumber}`,
