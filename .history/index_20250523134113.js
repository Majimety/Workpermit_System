...
const { sanitizeFilename, formatHumanDateTime } = require("./utils");
const fs = require("fs");
...
app.post("/sheet-updated", async (req, res) => {
  try {
    ...
    const humanDateTime = formatHumanDateTime(timestamp);
    const safeSubstationCode = sanitizeFilename(data.FORM_SUBSTATION);
    const safeNameSupervisor1 = sanitizeFilename(data.FORM_SV1);
    const filePath = `temp/${safeSubstationCode}/${safeNameSupervisor1}_${humanDateTime}.pdf`;
