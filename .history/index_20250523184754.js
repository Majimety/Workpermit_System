...
function generatePdfFile(data) {
  ...
  const safeAuditor = sanitizeFilename(data.Auditor);
  const filePath = `temp/${safeSubstationCode}/${safeAuditor}_${safeDateTime}.pdf`;
...
async function deleteOldFileIfExists(authClient, rowNumber) {
  ...
  const range = `Permit Form!AD${rowNumber}`;//ลบไฟล์เดิม
