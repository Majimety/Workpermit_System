...
async function getSheetData(auth, rowNumber = 2) {
  ...
  const RECIPIENT_Name = getMatchedValue(getValue(row, 37));
  const SIGNER_Name = getMatchedValue(getValue(row, 42));
  const PPSTAFF_Name = getValue(row, 44);
  ...
  return {
    ...
    SIGNATURE_PPSTAFF: getSignatureUrlByName(PPSTAFF_Name),
  };
}
