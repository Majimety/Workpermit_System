...
app.post("/sheet-updated", async (req, res) => {
  try {
    ...
    const fileName = `สถานีไฟฟ้า/${safeSubstationCode}/${safeNameSupervisor1}_${timestamp}.pdf`;
    const pdfPath = await createPdf(data, fileName);
    //--------------------------------------------------------------
    const fileId = await uploadToDrive(client, pdfPath);
    if (!fileId) throw new Error("PDF upload failed, no file ID returned.");
    ...
    const drive = google.drive({ version: "v3", auth: client });
      ...
      console.log(`Set permission: Anyone with the link can view`);
    const driveUrl = `https://drive.google.com/file/d/${fileId}/view?usp=sharing`;
    const sheets = google.sheets({ version: "v4", auth: client });
      await sheets.spreadsheets.values.update({ //URL_PDF to Google Sheets
