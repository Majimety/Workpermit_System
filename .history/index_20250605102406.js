...
app.post("/sheet-updated", async (req, res) => { //When trigger
  ...
  try {
    ...
    logger.error(`PDF generation failed for row ${req.body.triggerRow || 2}: ${err.message}`, { stack: err.stack });
    res.status(500).send("Internal Server Error");
  }
});

function generatePdfFile(data) {//Generate PDF
  logger.info("Generating PDF file name and starting PDF creation");
...
function cleanTempFolder() {//Delete folder PDF
  try {
