...
async function getSheetData(auth, rowNumber = 2) {
  ...
  const range = `'CREATE PDF Form'!A${rowNumber}:AP${rowNumber}`;
  ...
  return {
    ...
    FORM_STTIME: formatThaiTime(values[18]),
    ...
    FORM_CB_EXTINGUISH: values[13] == "ดับไฟปฏิบัติงาน",
    FORM_CB_NOEXTINGUISH: values[13] == "ไม่ดับไฟปฏิบัติงาน",
    FORM_CB_PLAN: values[15] == "นอกแผน",
    FORM_CB_NOPLAN: values[15] == "ตามแผน",
    FORM_CB_EMERGENCY: values[15] == "กรณีฉุกเฉิน (ศูนย์ฯ พิจารณา)",
    // ---------------------------------------
    FORM_EXECUTIVE: values[31],
    FORM_EXECUTIVE_VACANCY: values[32],
    // EXECUTIVE_TEMP: values[],
    // ---------------------------------------
    FORM_RECIPIENT: values[33],
    FORM_CB_ACCEPT: values[34] == "TRUE",
    FORM_CB_DENY: values[34] == "FALSE",
    FORM_DENY_CAUSE: values[35],
    // FORM_TO_SIGNER: values[],
    FORM_APPROVED: values[36],
    FORM_UNITS: values[37],
    // ---------------------------------------
    FORM_SIGNER: values[38],
    // ---------------------------------------
    PERFORMANCE_REPORT: values[39],
    FORM_PPSTAFF: values[40],
    FORM_PPSTAFF_VACANCY: values[41]
  };
}
