    ...
    if (existingUrl?.includes("drive.google.com")) {
      ...
      if (oldFileId) {
        ...
        try {
          ...
          if (err.code === 404) {
            console.warn(`⚠️ File not found: ${oldFileId} (already deleted)`);
          } else {
            console.error(`❌ Failed to delete file: ${oldFileId}`, err.message);
          }
        }
      } else {
        console.warn("⚠️ Could not extract file ID from URL:", existingUrl);
      }
    } else {
      console.log("ℹ️ No existing file to delete for row", rowNumber);
    }
  } catch (err) {
